...
<div>
    <v-container v-if="!loading">
        <v-layout xs12 justify-center align-content-center>
            <flower-spinner :animation-duration="2500" :size="70" color="#555A9C" class="load-spinner" />
        </v-layout>
    </v-container>
    <v-container v-if="loading">
        <!-- edit btn -->
        <v-layout xs12 justify-center class="page-edit" v-if="authCheck()">
            <v-flex xs12 text-xs-center my-5>
                <v-btn @click="newToggle = !newToggle" block flat large :color="newToggle? '#f0bebe':'#3a718c'">
                    <v-icon left>fa-chevron-down</v-icon>Edit Portfolio
                </v-btn>
            </v-flex>
        </v-layout>
        <!-- Portfolio Writer --->
        <transition name="fade">
            <v-layout v-if="newToggle" xs12 justify-center>
                <PortfolioWriter :id="id" :ti="title" :body="body" :imgSrc="imgSrc" :update=true />
            </v-layout>
        </transition>
        ...
        <v-layout class="page-img" v-if="this.imgSrc != ''">
            <v-flex xs12 justify-center>
                <v-img :src="this.imgSrc" cover></v-img>
            </v-flex>
        </v-layout>
        <!-- body -->
        <v-layout wrap>
...
export default {
    ...
    methods: {
        ...
        async deletePortfolio() {
            // DB에서 삭제 요청
            if (await FirebaseService.deletePortfolio(this.id)) {
                this.goToPortfolio()
            } else {
                alert('삭제권한이 없습니다.')
            }
