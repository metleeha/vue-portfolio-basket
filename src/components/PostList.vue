...
<v-container>
    ...
    <v-layout xs12 row wrap justify-center text-xs-center>
        <v-flex xs10>
            <v-text-field prepend-inner-icon="fa-search" v-model="search" placeholder="Search title.." style="width:50%;">
            </v-text-field>
        </v-flex>
    </v-layout>
    <!-- Post List -->
    <v-layout mt-5 xs12 row wrap id="pfPan">
        <v-flex xs10 v-for="i in filteredPosts.length > showPostLimits ? showPostLimits : filteredPosts.length" class="pflist" :key="i">
            <Post :id="filteredPosts[i - 1].id" :name="filteredPosts[i - 1].username" :date="new Date(filteredPosts[i - 1].created_at.toDate())" :title="filteredPosts[i - 1].title" :body="filteredPosts[i - 1].body"></Post>
            <v-divider></v-divider>
        </v-flex>
        <v-flex xs12 text-xs-center round my-5 v-if="loadMore">
            <v-btn color="#3a718c" dark large v-on:click="loadMorePosts()">
